...
@onready var guest_login_button = $VBoxContainer/GuestLoginButton
...
func _on_auth_success(user_info):
	status_label.text = "步骤4: 认证成功！欢迎 " + user_info.nickname
	await get_tree().create_timer(1.0).timeout
	# 主场景会自动切换到大厅界面

func _on_auth_failed(error_msg):
	status_label.text = "WebSocket认证失败: " + error_msg
	guest_login_button.disabled = false